...
def iterate(t,tolerance):
    ...
    if ((target - joint_positions['elbow']).cross(joint_positions['wrist'] - joint_positions['elbow']).length <= 0.1):
        arr = get_wrist_position([0, 0.1, 0.1, 0.1, 1])
        joint_positions['wrist'] = vector(arr)

    elif((target - joint_positions['elbow']).length <= 0.1):
        arr = get_wrist_position([0, 0.1, 0.1, 0.1, 1])
        joint_positions['wrist'] = vector(arr)
